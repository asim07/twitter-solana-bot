...
from construct import Int32ul, Int8ul, Int64ul
from solana.rpc.types import TokenAccountOpts
from solana.transaction import AccountMeta


from utils.raydium.layouts import SWAP_LAYOUT
...
def compute_units_price_inx(micro_lamports):
    # PROGRAM_ID = Pubkey.from_string("ComputeBudget111111111111111111111111111111")
    # keys = []
    # request_units = CStruct(
    #     "instruction" / Int8ul,                    
    #     "units" / Int64ul)
    # data = request_units.build({"instruction":3,"units":micro_lamports})
    # return Instruction(PROGRAM_ID, data, keys)
    return set_compute_unit_price(micro_lamports)

def compute_units_limit_inx(limit):
    # PROGRAM_ID = Pubkey.from_string("ComputeBudget111111111111111111111111111111")
    # keys = []
    # request_units = CStruct(
    #     "instruction" / Int8ul,                    
    #     "units" / Int32ul)
    # data = request_units.build({"instruction":2,"units":limit})
    # return Instruction(PROGRAM_ID, data, keys)
    return set_compute_unit_limit(limit)


def get_transfer_instruction(from_: str, to: str, amount: float):
    params = TransferParams(from_pubkey=Pubkey.from_string(from_),
...
def make_swap_instruction(amount_in: int, token_account_in: Pubkey.from_string, token_account_out: Pubkey.from_string,
                              accounts: dict, mint, ctx, owner, minAmountOut: int) -> Instruction:
        tokenPk = mint
        ...
        keys = [
            ...
            AccountMeta(pubkey=Pubkey.from_string(accounts["amm_id"]), is_signer=False, is_writable=True),
            AccountMeta(pubkey=Pubkey.from_string(accounts["authority"]), is_signer=False, is_writable=False),
            AccountMeta(pubkey=Pubkey.from_string(accounts["open_orders"]), is_signer=False, is_writable=True),
            AccountMeta(pubkey=Pubkey.from_string(accounts["target_orders"]), is_signer=False, is_writable=True),
            AccountMeta(pubkey=Pubkey.from_string(accounts["base_vault"]), is_signer=False, is_writable=True),
            AccountMeta(pubkey=Pubkey.from_string(accounts["quote_vault"]), is_signer=False, is_writable=True),
            AccountMeta(pubkey=SERUM_PROGRAM_ID, is_signer=False, is_writable=False), 
            AccountMeta(pubkey=Pubkey.from_string(accounts["market_id"]), is_signer=False, is_writable=True),
            AccountMeta(pubkey=Pubkey.from_string(accounts["bids"]), is_signer=False, is_writable=True),
            AccountMeta(pubkey=Pubkey.from_string(accounts["asks"]), is_signer=False, is_writable=True),
            AccountMeta(pubkey=Pubkey.from_string(accounts["event_queue"]), is_signer=False, is_writable=True),
            AccountMeta(pubkey=Pubkey.from_string(accounts["market_base_vault"]), is_signer=False, is_writable=True),
            AccountMeta(pubkey=Pubkey.from_string(accounts["market_quote_vault"]), is_signer=False, is_writable=True),
            AccountMeta(pubkey=Pubkey.from_string(accounts["market_authority"]), is_signer=False, is_writable=False),
        ...
        data = SWAP_LAYOUT.build(
            dict(
                ...
                min_amount_out=int(minAmountOut)
            )
        )
